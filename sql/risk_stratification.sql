...
Grain: (member_id, year)
Includes exposure (member_months), cost (total_allowed_med),
ED utilization (ed_visits), and exposure-adjusted PMPM (pmpm_med).
----------------------------------------------------------------- */
...
Flags members whose annual allowed spend is in the top 1% within each year
----------------------------------------------------------------------------- */
...
Flags members whose PMPM increased meaningfully from 2024 -> 2025.
Exposure thresholds reduce noise: >=6 months in both years.
Rising logic: absolute increase >= $150 OR relative increase >= 25%
----------------------------------------------------------------------------- */
...
Flags members with high ED utilization with exposure minimum:
member_months >= 6 AND ed_visits >= 4
========================================================================== */
...
SELECT
    b.*,
    CASE
        WHEN b.member_months >= 6 AND b.ed_visits >= 4 THEN 1 ELSE 0
    END AS is_ed_superutilizer
FROM dbo.vw_member_year_base b;
GO

/* ===========================================================================
5) vw_member_targeting
Unified Power BI-ready output with flags + tier label + member attributes.
============================================================================ */
